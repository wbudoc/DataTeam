####################################################################################################################################
#
#          All account and address report and email1 email2 email3 and no full household and company contact
#                                              INT-15
#
####################################################################################################################################
...
SELECT a.id `Account ID`,NULL HeadAccountID,IF(a.isCompanyAccount=1,'Yes','No') IsCompanyAccount,u.userId,u.prefix Prefix,u.firstName `First Name`,u.middleName `Middle Name`,IF(u.userType=3,NULL,u.lastName) `Last Name`,u.suffix Suffix,a.houseHoldName `Household Name`,
...
UNION
SELECT NULL,u.accountId HeadAccountID,'No',u.userId,u.prefix,u.firstName,u.middleName,u.lastName,u.suffix,h.name,h.houseHoldSalutation,NULL,IF(u.deceased=1,'Yes','No') `Deceased`,NULL,REPLACE(u.email1,'.z2','') email1,REPLACE(u.email2,'.z2','') email2,REPLACE(u.email3,'.z2','') email3 FROM user u
...
SELECT zn.`Account ID`,zn.HeadAccountID,zn.IsCompanyAccount,zn.Prefix,zn.`First Name`,zn.`Middle Name`,zn.`Last Name`,zn.Suffix,zn.`Household Name`,zn.`Household Salutation`,zn.`Company Name`,zn.`Deceased`,zn.`Do Not Contact`,zn.email1,zn.email2,zn.email3,
...
SELECT * FROM z_newcreate_individual_and_organization_report_20200710 ORDER BY IF(`Account ID`+0 IS NULL,HeadAccountID+0,`Account ID`+0);
