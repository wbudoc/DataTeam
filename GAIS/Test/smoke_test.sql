...
SELECT a.description,a.result,SUM(a.number),"SELECT * FROM z_newcreate_template_gais WHERE parentType='Activity' AND LENGTH(TRIM(gaisLegacyId))>0 GROUP BY gaisLegacyId,sourceTable,gaisLegacyIdSeq HAVING COUNT(*)>1;" FROM (SELECT 'Duplicate legacyId in Activity' description,IF(COUNT(*)>0,'Fail','Pass') result,COUNT(*) number FROM z_newcreate_template_gais WHERE parentType='Activity' AND LENGTH(TRIM(gaisLegacyId))>0 GROUP BY gaisLegacyId,sourceTable,gaisLegacyIdSeq HAVING COUNT(*)>1) a;
...
##############################################################################################################################################
INSERT INTO smoke_test_report(errorMsg,passFlag,errorCount,sqlStatement)
SELECT a.description,a.result,SUM(a.number,"SELECT * FROM z_newcreate_template_gais WHERE parentType='Grants' AND LENGTH(TRIM(gaisLegacyId))>0 GROUP BY gaisLegacyId,sourceTable,gaisLegacyIdSeq HAVING COUNT(*)>1;" FROM (SELECT 'Duplicate legacyId in Grants' description,IF(COUNT(*)>0,'Fail','Pass') result,COUNT(*) number FROM z_newcreate_template_gais WHERE parentType='Grants' AND LENGTH(TRIM(gaisLegacyId))>0 GROUP BY gaisLegacyId,sourceTable,gaisLegacyIdSeq HAVING COUNT(*)>1) a;
...
##############################################################################################################################################
INSERT INTO smoke_test_report(errorMsg,passFlag,errorCount,sqlStatement)
SELECT a.description,a.result,SUM(a.number,"SELECT * FROM z_newcreate_template_gais WHERE parentType='Invitation' AND LENGTH(TRIM(gaisLegacyId))>0 GROUP BY gaisLegacyId,sourceTable,gaisLegacyIdSeq HAVING COUNT(*)>1;" FROM (SELECT 'Duplicate legacyId in Invitation' description,IF(COUNT(*)>0,'Fail','Pass') result,COUNT(*) number FROM z_newcreate_template_gais WHERE parentType='Invitation' AND LENGTH(TRIM(gaisLegacyId))>0 GROUP BY gaisLegacyId,sourceTable,gaisLegacyIdSeq HAVING COUNT(*)>1) a;
...
##############################################################################################################################################
INSERT INTO smoke_test_report(errorMsg,passFlag,errorCount,sqlStatement)
SELECT a.description,a.result,SUM(a.number),"SELECT * FROM z_newcreate_template_gais WHERE parentType='Prospects' AND LENGTH(TRIM(gaisLegacyId))>0 GROUP BY gaisLegacyId,sourceTable,gaisLegacyIdSeq HAVING COUNT(*)>1;" FROM (SELECT 'Duplicate legacyId in Prospects' description,IF(COUNT(*)>0,'Fail','Pass') result,COUNT(*) number FROM z_newcreate_template_gais WHERE parentType='Prospects' AND LENGTH(TRIM(gaisLegacyId))>0 GROUP BY gaisLegacyId,sourceTable,gaisLegacyIdSeq HAVING COUNT(*)>1) a;
