...
INSERT INTO smoke_test_report(errorMsg,passFlag,errorCount,sqlStatement)
SELECT 'Priority is blank, and we will set it as normal in Activity',IF(COUNT(*)>0,'Warning','Pass'),COUNT(*),"SELECT * FROM z_newcreate_template_gais WHERE parentType='Activity' AND (LENGTH(TRIM(activityPriority))=0 OR activityPriority IS NULL);" FROM z_newcreate_template_gais WHERE parentType='Activity' AND (LENGTH(TRIM(activityPriority))=0 OR activityPriority IS NULL);
